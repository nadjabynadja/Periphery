...
def score_cluster(
    ...
    """Score coherence of a cluster by averaging pairwise scores (legacy)."""
...
def score_document(
    ...
    """Score how well a document fits within its cluster (legacy)."""
    if cluster_vectors.shape[0] == 0:
        return 0.0

    model.eval()
    n = min(cluster_vectors.shape[0], 50)
...
def score_all_clusters(
    ...
    """Score coherence for all clusters (legacy)."""
    scores = {}
