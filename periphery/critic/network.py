...
class CoherenceNet(nn.Module):
    """Legacy pair-based coherence scoring network.

    Takes concatenated embedding pairs (2 * dim) and outputs a coherence score.
    Kept for backward compatibility with the legacy scoring pipeline.
    """
...
class StructuralDiscriminator(nn.Module):
    """Legacy discriminator kept for backward compatibility.

    The new architecture uses CoherenceCritic directly with supervised
    training on perturbation data. This class is preserved so existing
    code that references it doesn't break.
    """
